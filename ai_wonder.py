...
from input_handler import input_piped_model, encoder_name             # Just to export outside
...
def read_dataset(source, drop_unnamed_cols=True):
    ...
    # Convert every ".0" float columns into ints
...
def local_explanations(state, orig, form="as_list"):
    ...
    def listify_explanation(exp):
        ...
        if encoder_name(state).startswith("OneHot"):
            point_dict = forward_transform(state, orig, look_cats=True).to_dict(orient='records')
        else:
            point_dict = orig.to_dict(orient='records')
